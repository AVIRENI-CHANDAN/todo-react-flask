...
class TestTodo(unittest.TestCase):
    ...
    def test_create_todo(self):
        response = self.app.post(
            "/task", json={"title": "Test todo", "description": "Test todo description"}
        )
    ...
    def test_get_todos(self):
        ...
        todo2 = Todo(
            title="Test todo 2", description="Test description 2", completed=True
        )
    ...
    def test_update_todo(self):
        with app.app_context():
            ...
            response = self.app.put(
                f"/task/{todo.id}", json={"title": "Updated todo", "completed": True}
            )
            self.assertEqual(response.status_code, 200)
            print(response.json)
            ...
            response = self.app.put(
                f"/task/{todo.id}", json={"title": "Updated 2 todo", "completed": False}
            )
            self.assertEqual(response.status_code, 200)
            print(response.json)
